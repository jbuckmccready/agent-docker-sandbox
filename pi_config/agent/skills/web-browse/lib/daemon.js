...
export async function runWebBrowseDaemon({
  ...
  console.error(`Starting web-browse daemon on ${daemonUrl} (headless Chromium + CDP)...`);

  const browserProcess = await startBrowserForCdp(preferredCdpPort, cdpProfile, browserBinArg);
  console.error(`Chromium started for daemon (pid=${browserProcess.proc.pid}, cdpPort=${browserProcess.port})`);
    ...
    if (req.method === "GET" && req.url === "/health") {
      ...
      res.end(
        JSON.stringify({
          status: "ok",
          pid: process.pid,
          browserPid: browserProcess.proc.pid,
