  ...
  try {
    ...
    const extractResultsFromDocument = () => {
      ...
      for (const titleEl of titleEls) {
        ...
        let snippet = "";
        const container =
          linkEl.closest("div.MjjYud, div.g, div[data-snc]") || linkEl.parentElement?.parentElement;

        if (container) {
          const snippetEl = container.querySelector("[data-snf='nke7rc'], .VwiC3b, .yXK7lf, .lEBKkf, span.aCOpRe");
          snippet = snippetEl?.textContent?.trim().replace(/Read more$/, "").trim() || "";
